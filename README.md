...
- `domain/`：兩位玩家、純資料規則、集中式 Zone／牌庫／裝備服務、FIFO 效果解析、資源 evaluator、命令、RNG、invariants；不得依賴 Node 或 Presentation。
...
目前的 vertical slice 使用 placeholder 資產證明：官方起始配置 → 兩位玩家五階段輪替 → `EQUIP_ITEM` 與雙向附件 → 手牌購買力／隊伍戰力 evaluator → 休息棄牌／洗牌／抽牌 → Legal Commands → transaction → events → 3D／2D 呈現 → Snapshot/hash。
