...
目前的 vertical slice 使用 placeholder 資產證明：官方起始配置 → 兩位玩家五階段輪替 → 招募區與商店各公開三張 → `BUY_CARD` 購買後進棄牌 → `REFRESH_MARKET` 棄 1 張手牌並更換同列 1～3 張 → 休息補列／棄牌／洗牌／抽牌 → `PLAY_ADVENTURER` 滿編替換 → `EQUIP_ITEM` 與雙向附件 → `USE_ITEM` 抽牌 → 手牌購買力／隊伍戰力 evaluator → Legal Commands → transaction → events → 3D／2D 呈現 → Snapshot/hash。

目前刻意未納入的是完整官方卡池與討伐結算；`docs/` 是本機企劃資料，已由 `.gitignore` 排除。
